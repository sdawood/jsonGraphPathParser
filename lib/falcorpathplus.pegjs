    ...
    'member-child-identifier' : partial(
        {
          operation: 'member',
          scope: 'child',
          expression: { type: 'identifier', value: 'title' }
        }
      )
    }
...
PATH_COMPONENTS
  = first:(
      parent:MEMBER_EXPRESSION? {
        return ast("MEMBER_EXPRESSION").set(jp['member-child-identifier']({ expression: { value: parent } }));
      }
    )
    rest:(
...
CHILD_MEMBER_COMPONENT
  = DOT member:MEMBER_EXPRESSION { return [ ast("PATH_COMPONENT").set(jp['member-child-identifier']({ expression: { value: member } })) ]; }
